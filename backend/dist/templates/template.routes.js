...
export async function templateRoutes(app) {
    ...
    app.get('/api/templates/discover/:hypervisorId', async (request, reply) => {
        const { hypervisorId } = request.params;
        const db = getDb();
    ...
    app.post('/api/templates', async (request) => {
        ...
        const db = getDb();
        const result = db.prepare('INSERT INTO vm_templates (hypervisor_id, source_vm_id, name, description, default_cores, default_memory_mb, default_disk_gb, os_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(body.hypervisor_id, body.source_vm_id, body.name, body.description || null, body.default_cores, body.default_memory_mb, body.default_disk_gb, body.os_type || null);
