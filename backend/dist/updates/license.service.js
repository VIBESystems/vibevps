...
export function getOrCreateServerId() {
    const db = getDb();
    const row = db.prepare('SELECT value FROM settings WHERE key = ?').get('server_id');
    if (row)
        return row.value;
...
export async function activateLicense(licenseKey, customerEmail) {
    ...
    `).run(serverId, licenseKey, data.customer_name || null, customerEmail, data.plan || null, data.max_hypervisors || 0, data.max_vms || 0, data.expires_at || null, data.is_lifetime ? 1 : 0);
...
export async function verifyLicense() {
    ...
    try {
        ...
        WHERE id = 1
      `).run(data.plan || null, data.max_hypervisors || 0, data.max_vms || 0, data.expires_at || null, data.is_lifetime ? 1 : 0, data.customer_name || null);
        }
...
export async function checkForUpdates() {
    ...
    try {
        ...
        const data = await response.json();
        // Normalize changelog
        let normalizedChangelog = [];
        if (Array.isArray(data.changelog)) {
            normalizedChangelog = data.changelog.map((entry) => ({
            ...
            try {
                ...
                if (Array.isArray(parsed)) {
                    normalizedChangelog = parsed.map((entry) => ({
                        version: entry.version,
                        min_version_required: entry.min_version_required || entry.min_version || '1.0.0',
                        date: entry.date,
                        changes: entry.changes || [],
