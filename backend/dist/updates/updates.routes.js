...
export async function updatesRoutes(app) {
    ...
    app.post('/api/license/activate', async (request, reply) => {
        const { licenseKey, customerEmail } = request.body;
    ...
    app.post('/api/updates/install', async (request, reply) => {
        const { downloadTokens, targetVersion, changelog } = request.body;
        ...
        let versionHistory = { current_version: currentVersion, applied_updates: [] };
        try {
        ...
        exec(command, { cwd: INSTALL_DIR }, (error) => {
            if (error)
                console.error('[Update] Failed to launch script:', error);
        });
        return {
            success: true,
