...
export class ProxmoxAdapter {
    ...
    async listVMs() {
        ...
        return (data || []).map((vm) => this.mapVM(vm));
    }
    async getVM(vmId) {
    ...
    async getVMIp(vmId) {
        const interfaces = await this.request('GET', `/nodes/${this.node}/qemu/${vmId}/agent/network-get-interfaces`);
        if (!Array.isArray(interfaces?.result))
            return undefined;
    ...
    async uploadVendorData(vmId, snippetStorage) {
    ...
    async cloneFromTemplate(cfg) {
        ...
        const hasCloudInit = Object.entries(currentConfig).some(([_, v]) => String(v).includes('cloudinit'));
    ...
    async postBootSetup(vmId, diskDev, expandDisk) {
    ...
    async waitForTaskComplete(upid, maxWait = 300000) {
...
//# sourceMappingURL=proxmox.adapter.js.map