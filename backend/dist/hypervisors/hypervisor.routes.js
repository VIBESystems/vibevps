...
async function autoDetectNode(host, port, tokenId, tokenSecret) {
...
export async function hypervisorRoutes(app) {
    ...
    app.post('/api/hypervisors', async (request, reply) => {
        ...
        const result = db.prepare('INSERT INTO hypervisors (name, type, host, port, node, api_token_id, api_token_secret, verify_ssl) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(body.name, body.type, body.host, body.port, body.node, body.api_token_id, body.api_token_secret, body.verify_ssl ? 1 : 0);
    ...
    app.get('/api/hypervisors/:id/test', async (request, reply) => {
        const { id } = request.params;
        const db = getDb();
        const hv = db.prepare('SELECT * FROM hypervisors WHERE id = ?').get(id);
        if (!hv)
            return reply.status(404).send({ error: 'Hypervisor non trovato' });
        try {
            clearAdapterCache(Number(id));
...
//# sourceMappingURL=hypervisor.routes.js.map