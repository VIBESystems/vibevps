...
function getHypervisor(id) {
    const db = getDb();
    const hv = db.prepare('SELECT * FROM hypervisors WHERE id = ?').get(id);
    if (!hv)
        throw new Error('Hypervisor non trovato');
    return hv;
}
function logAction(hypervisorId, vmId, vmName, action, details, status = 'success') {
    const db = getDb();
...
export async function vmRoutes(app) {
    ...
    app.get('/api/vms/:hypervisorId/:vmId', async (request, reply) => {
        const { hypervisorId, vmId } = request.params;
        const hv = getHypervisor(Number(hypervisorId));
        const adapter = getAdapter(hv);
        return adapter.getVM(vmId);
    });
    // VM actions
    for (const action of ['start', 'stop', 'restart', 'suspend']) {
    ...
    app.post('/api/vms/create', async (request, reply) => {
        ...
        if (body.template_id && !templateVmId) {
            const db = getDb();
            const tmpl = db.prepare('SELECT source_vm_id FROM vm_templates WHERE id = ?').get(body.template_id);
            if (!tmpl)
                return reply.status(404).send({ error: 'Template non trovato' });
...
//# sourceMappingURL=vm.routes.js.map